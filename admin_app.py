...
def process_music_generation(task_id, data):
    ...
    try:
        ...
        # Take the first clip (Suno typically generates 2 clips)
        clip = audio_clips[0]
        
        result = {
            'success': True,
            'title': clip.get('title', data.get('song_title', f"Generated {data.get('genre_specific', 'Track')}")),
            'audio_url': clip.get('streamAudioUrl') or clip.get('audioUrl') or clip.get('sourceStreamAudioUrl'),
            'video_url': clip.get('imageUrl') or clip.get('sourceImageUrl'),  # Suno provides album art as video/image
