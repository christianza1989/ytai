...
class VideoCreator:
    ...
    def create_video_from_audio_and_image(self, audio_path: str, image_path: str, 
        ...
        """
        Create video from local audio and image files
        
        Args:
            audio_path: Local path to audio file
            image_path: Local path to image file  
            output_path: Directory where to save the video (NOT the full file path)
            title: Video title for metadata
        ...
        """
        try:
            print(f"🎥 Creating video from local files: {title}")
            ...
            video_file_path = Path(output_path) / f"{safe_title}.mp4"
            ...
            if result.returncode == 0:
                ...
                print(f"✅ Video created successfully!")
                print(f"📁 Output: {video_file_path}")
                print(f"📊 Size: {file_size_mb:.1f} MB")
                print(f"⏱️ Encoding time: {encode_time:.1f}s")
